...
function SocialLinks({ linkedin, github }: { linkedin: string; github: string }) {
  return (
    <div className="flex justify-center gap-6 pt-4 border-t border-red-500/20">
      ...
      >
        <img src={logoLinkedin} alt="LinkedIn" className="w-7 h-7 invert" />
      </a>
      <a 
        href={github} 
        target="_blank" 
        ...
        className="transform hover:scale-110 transition duration-300 hover:filter hover:drop-shadow-[0_0_8px_#ef4444]"
      >
        <img src={logoGithub} alt="GitHub" className="w-7 h-7 invert" />
      </a>
    </div>
  );
}

function MemberCard({ membro }: { membro: IntegranteType }) {
  return (
    <div className="relative group transform hover:scale-[1.03] transition-all duration-500">
      ...
      <div className="relative bg-black/80 backdrop-blur-sm border border-red-500/30 rounded-2xl p-8 group-hover:border-red-500/60 transition-all duration-300 h-full flex flex-col items-center">
        ...
        <div className="text-center flex-1">
          ...
          <SocialLinks linkedin={membro.linkedin} github={membro.github} />
        </div>
      </div>
